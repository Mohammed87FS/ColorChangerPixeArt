...
class ColorChangerApp:
    ...
    def setup_gui(self):
        ...
        self.detect_objects_button = ttk.Button(color_picker_frame, text="Detect Objects", command=self.detect_objects)
        self.detect_objects_button.grid(row=0, column=2, padx=5, pady=5)

        self.suggest_colors_button = ttk.Button(color_picker_frame, text="Suggest Colors", command=self.suggest_colors)
        self.suggest_colors_button.grid(row=1, column=2, padx=5, pady=5)

        self.confidence_slider = tk.Scale(color_picker_frame, from_=0, to=1, resolution=0.01, orient=tk.HORIZONTAL, label="Confidence Threshold", command=self.update_confidence_threshold)
        self.confidence_slider.set(self.confidence_threshold)
        self.confidence_slider.grid(row=1, column=0, columnspan=3, padx=5, pady=5)
        ...
        if self.image:
            ...
            for i in range(len(masks)):
                if scores[i] > self.confidence_threshold:
                    ...
                    self.segmented_objects.append((mask, box))

            self.display_segmented_objects()

    def display_segmented_objects(self):
        image_np = np.array(self.image)
        for mask, box in self.segmented_objects:
            color = [0, 255, 0]  # Green color for the mask
            for c in range(3):
                image_np[:, :, c] = np.where(mask == 255, image_np[:, :, c] * 0.5 + color[c] * 0.5, image_np[:, :, c])
    ...
    def select_object(self, event):
        if self.segmented_objects:
            x = int(event.x / self.zoom_factor)
            y = int(event.y / self.zoom_factor)
            print(f"Click coordinates: ({x}, {y})")  # Debug print
            for mask, box in self.segmented_objects:
                print(f"Object box: {box}")  # Debug print
                if box[0] <= x <= box[2] and box[1] <= y <= box[3]:
                    self.object_masks = [mask]
                    self.status_label.config(text="Status: Object selected")
                    self.highlight_selected_object(mask)
                    return
            self.status_label.config(text="Status: No object selected")

    def highlight_selected_object(self, mask):
        image_np = np.array(self.image)
        color = [255, 0, 0]  # Red color for highlighting
        for c in range(3):
            image_np[:, :, c] = np.where(mask == 255, color[c], image_np[:, :, c])
        self.image = Image.fromarray(image_np)
        self.display_image()
    ...
    def process_image(self):
        ...
        if not self.object_masks:
            messagebox.showerror("Error", "Please detect objects and select an object.")
            return
        if not self.new_color:
            messagebox.showerror("Error", "Please pick a new color.")
            return
        ...
        for mask in self.object_masks:
            for c in range(3):
                img_np[:, :, c] = np.where(mask == 255, new_color_np[c], img_np[:, :, c])
